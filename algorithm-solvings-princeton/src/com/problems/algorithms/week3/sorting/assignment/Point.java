...
public class Point implements Comparable<Point> {
	...
	public int compareTo(Point that) {
		...
		if (this.y > that.y) {
			...
			//if (this.x < that.x) {
			//changed to below condition for convex hull applications
			if (this.x > that.x) {
				return -1;
...
//				if(slopeTo(q1) == Double.POSITIVE_INFINITY || slopeTo(q2) == Double.POSITIVE_INFINITY) {
//					return -1;
//				}
	...
	public boolean isCCW(Point p, Point q) {
		if (this.y < p.y && this.x >= p.x && p.x < q.x)
			return true;
		else if (this.x < p.x && p.y > q.y)
			return true;
		return false;
	}
	// 
	/**
	 * 
	 * Geometrical way to find the points are Counter clock wise
	 * if area > o then counter clock wise
	 * @param a
	 * @param b
	 * @param c
	 * @return
